...
const CategoryController = () => {
    ...
    const handleExportData = () => {
        ...
        const headers = ['اسم الصنف', 'نوع الصنف', 'الفرع', 'الفئة الرئيسية', 'الفئة الفرعية', 'الوحدة الرئيسية', 'الوحدة الفرعية', 'الباركود', 'كود الصنف', 'الحد الأدنى', 'الحد الأقصى', 'نقطة إعادة الطلب', 'سعر التكلفة', 'سعر البيع', 'معدل الضريبة', 'المورد', 'الحالة'];
        const csvContent = [
            headers.join(','),
    ...
    const handleFileUpload = (event) => {
        ...
        if (file) {
            ...
            reader.onload = (e) => {
                ...
                const newCategories = rows.slice(1).filter(row => row.trim()).map((row, index) => {
                                    ...
                                    <Stack spacing={2}>
                                        ...
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                            ...
                                            <Chip 
                                                ...
                                                color={selectedCategory.status === 'نشط' ? 'success' : selectedCategory.status === 'غير نشط' ? 'warning' : 'error'}
